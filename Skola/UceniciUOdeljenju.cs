...
namespace Skola
{
    public partial class UceniciUOdeljenju : Form
    {
        ...
        private void btnDodaj_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Da li ste sifurni da ste dodali sve predmete za odeljenje?", "Potvrdi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    string id = txtId.Text;
                    string ime = txtIme.Text;
                    string prezime = txtPrezime.Text;
                    if (id == String.Empty || ime == String.Empty || prezime == String.Empty)
                    {
                        MessageBox.Show("Niste uneli sve podatke za ucenika");
                    }
                    else
                    {
                        DataProvider.DodajUcenika(id, ime, prezime, odeljenjeID, "5");
                        ...
                        btnObrisiPredmet.Enabled = false;
                    }
                }
            }
            else
        ...
        private void btnObrisi_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Niste selektovali ucenika");
            }
            else
            {
        ...
        private void btnPremestaj_Click(object sender, EventArgs e)
        {
            ...
            else if(cbxOdeljenja.SelectedItem == null)
            {
                MessageBox.Show("Niste izabrali novo odeljenje");
            }
            else
            {
                string ucenikID = listView1.SelectedItems[0].Text;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (cbxPredmeti.SelectedItem != null && cbxProfesori.SelectedItem != null)
            {
                ...
                listView2.Items.Add(item);
                cbxProfesori.Items.Remove(cbxProfesori.SelectedItem);
                cbxPredmeti.Items.Remove(cbxPredmeti.SelectedItem);
                cbxPredmeti.Text = "Izaberi predmet";
                cbxProfesori.Text = "Izaberi profesora";
            }
            else
            {
        ...
        private void btnObrisiPredmet_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Niste selektovali ni jedan predmet")
            }
            else
            {
        ...
        {
            Ucenik uc = DataProvider.VratiUcenika(listView1.SelectedItems[0].SubItems[0].Text);
            RazredniOceneForm rof = new RazredniOceneForm(uc);
            rof.ShowDialog();
        }
